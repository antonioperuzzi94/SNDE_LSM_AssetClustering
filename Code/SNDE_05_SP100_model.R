...
for(it in 1:iterations){
  ...
  for(t in 1:Time){
   ...
   #sample gamma
    ...
    ht<-ht_list[t]
    prop_ht<- rnorm(1, ht, 1.5)
    
    
    if(t == 1){
      
      htp1<-ht_list[t+1]
      priorProp = dnorm(prop_ht, 1,  0.2, log =T ) + dnorm( htp1, prop_ht, 0.2, log =T )
      priorActual =  dnorm( ht,  1,  0.2, log =T )   + dnorm(htp1,  ht,      0.2, log =T ) 
      ...
      priorProp = dnorm(prop_ht, htm1 ,      0.2, log =T )  + dnorm( htp1,  prop_ht,  0.2, log =T )
      priorActual =  dnorm( ht, htm1 ,       0.2, log =T )    + dnorm( htp1,  ht,     0.2, log =T )
      
    } else if(t> 1 & t == Time){
      
      htm1<-ht_list[t-1]
      
      priorProp =    dnorm(prop_ht,   htm1 ,    0.2, log =T )  
      priorActual =  dnorm( ht,       htm1 ,    0.2, log =T )  
...
save.image("~/Documents/Github/LS_AssetClustering_SNDE_CODE_DATA/Data/chapter_res_dyn_rw_SP100.RData")
