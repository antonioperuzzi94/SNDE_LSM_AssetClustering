  ...
  for(it in 1:iterations){
    ...
    for(gg in 1:G){
      ...
      if(sum(belong == gg) == 0){   
        ...
        s2g1 = (1/2)*sum((zi_mat[,1]-   mu_mat[1])^2 + (zi_mat[,2]-   mu_mat[2])^2)
    ...
    sigma_list2[[t]][[it]] <- c(unlist(listS1), it)
...
save.image("~/Documents/Github/LS_AssetClustering_SNDE_CODE_DATA/Data/chapter_res_dyn_sequential_DAX40.RData")
